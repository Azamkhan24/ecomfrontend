...
const VerticalCardProduct = ({ category, heading }) => {
    ...
    return (
        <div className="container mx-auto px-4 my-6 relative " id='vertical-card-product'>
            <h2 className="text-2xl font-semibold py-4">{heading}</h2>
            {error && <p className="text-red-600">{error}</p>}
            <div className="flex items-center gap-4 md:gap-14 flex-wrap justify-center" ref={scrollElement}>
                {/* {data.length > 4 && (
                    ...
                    data.map((product, index) => (
                        <Link to={`product/${product._id}`} key={index} className="w-full min-w-[280px] md:min-w-[320px] max-w-[280px] md:max-w-[320px] bg-white rounded-sm shadow">
                            ...
                            <div className="p-4 grid gap-2">
                                ...
                                <p className="capitalize text-slate-500">{product?.aboutProduct?.description}</p>
